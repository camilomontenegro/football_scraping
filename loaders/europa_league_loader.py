"""
loaders/europa_league.loader.py
============================
Carga los datos de la Europa League en la base de datos.
"""
import logging
...
def _get_competition_id(conn) -> int:
    """Obtiene el canonical_id de la Europa League en dim_competition."""
    return conn.execute(text(
        "SELECT canonical_id FROM dim_competition WHERE id_transfermarkt = 'EL'"
    )).scalar()
    ...
    while opcion != "4":
        ...
        if opcion == "1":
            ...
            with engine.begin() as conn:
                load_teams(conn, ss_path=SS_EUROPA_LEAGUE, tm_path=TM_EUROPA_LEAGUE, ws_path=WS_EUROPA_LEAGUE)
            log.info("Teams completado.")
            log.info("-"*50)
        elif opcion == "2":
            log.info("Cargando players...")
            with engine.begin() as conn:
                load_players(conn, tm_path=TM_EUROPA_LEAGUE, ss_path=SS_EUROPA_LEAGUE, ws_path=WS_EUROPA_LEAGUE)
            log.info("Players completado.")
            log.info("-"*50)
        elif opcion == "3":
            log.info("Cargando matches...")
            with engine.begin() as conn:
                load_matches(conn, ss_path=SS_EUROPA_LEAGUE, competition_id=competition_id, ws_path=WS_EUROPA_LEAGUE, )
...
def _load_facts(competition_id: int) -> None:
    ...
    while opcion != "4":
        ...
        if opcion == "1":
            ...
            with engine.begin() as conn:
                load_shots(conn, ss_path=SS_EUROPA_LEAGUE , competition_id=competition_id)
            log.info("Shots completado.")
            log.info("-"*50)
        elif opcion == "2":
            log.info("Cargando events...")
            with engine.begin() as conn:
                load_events(conn, ss_path=SS_EUROPA_LEAGUE, ws_path=WS_EUROPA_LEAGUE)
            log.info("Events completado.")
            log.info("-"*50)
        elif opcion == "3":
            log.info("Cargando injuries...")
            with engine.begin() as conn:
                load_injuries(conn, tm_path=TM_EUROPA_LEAGUE)
...
def main() -> None:
    ...
    _setup_logging("europa_league.loader.log")
