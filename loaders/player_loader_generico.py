...
def _load_phase1_transfermarkt(conn, tm_path: Path) -> int:
    ...
    """
    # busca recursivamente cualquier CSV con "players" en el nombre
    files = list(tm_path.glob("**/*players*.csv"))

    if not files:
        log.warning("player_loader fase 1: no se encontró archivo csv con jugadores en %s", tm_path)
        return 0
...
def _load_phase2_sofascore(conn, ss_path: Path) -> tuple[int, int]:
    ...
    if not files:
        log.info("player_loader fase 2: no hay archivos de players de SofaScore en %s", ss_path)
        return 0, 0
...
def _load_phase3_understat(conn, us_path: Path) -> tuple[int, int]:
    ...
    try:
        dfs = [pd.read_csv(_f) for _f in files]
