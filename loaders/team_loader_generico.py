...
def _load_from_transfermarkt(conn,tm_path: Path) -> int:
    ...
    # busca recursivamente cualquier CSV con "teams" en el nombre
    files = list(tm_path.glob("**/*teams*.csv"))

    if not files:
        log.info("team_loader: no hay archivos de teams de Transfermarkt en %s", tm_path)
        return 0
    ...
    for f in files:
        try:
            ...
            for _, row in df.iterrows():
                ...
                # puede ocurrir que el campo country en el csv tenga valor NaN.
                # En ese caso, se trata como None para evitar excepción en la inserción en dim_team.
...
def _load_from_understat(conn, us_path: Path) -> int:
    """Lee cualquier *teams*.csv de Understat → añade id_understat a dim_team."""
...
def _load_from_statsbomb(conn, sb_path: Path) -> int:
    ...
    if not files:
        log.info("team_loader: no hay archivos de teams de StatsBomb en %s", sb_path)
        return 0
...
def _load_from_whoscored(conn, ws_path: Path) -> int:
    ...
    """Lee cualquier *teams*.csv de WhoScored → añade id_whoscored a dim_team."""
