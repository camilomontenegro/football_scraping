...
# descargar solo 2023 y 2024 de Premier League
python -m scrapers.transfermarkt_scraper --competition "Premier League" --seasons 2023 2024
...
# libreria para  manejar argumentos por linea de comandos 
import argparse
...
def _setup_logging(output_dir: str) -> None:
    """
    Configura el sistema de logging para escribir simultáneamente
    en consola y en un archivo de log en el directorio de salida.

    El archivo scraper.log registra todos los mensajes de nivel INFO
    o superior — útil para revisar errores después de una ejecución larga.

    Parámetros:
        output_dir (str): directorio donde se creará scraper.log
    """
...
def _load_config(competition_name: str) -> dict:
    """ 
        Carga la configuración de Transfermarkt para una competición.
        Para ello utiliza el diccionario de competiciones definido en competitios.py para  obtener el nombre, código y slug que tiene la competición en la web transfermarkt 

        Devuelve un diccionario  con el código, slug y nombre de la competición.
    """
    comp = get_competition(competition_name)
    if comp is None:
        raise ValueError(f"Competición '{competition_name}' no encontrada en competitions.py.")
    
    source = comp["sources"].get("transfermarkt",{})
    if not source:
        raise ValueError(f"'{competition_name}' no tiene configuración de Transfermarkt.")
    
    slug = source.get("slug")
    if not slug:
        raise ValueError(f"'{competition_name}' no tiene slug de Transfermarkt.")
...
def _append_to_csv(records: list[dict], path: str) -> None:
    ...
    Parámetros:
        ...
        path    (str):        ruta del archivo CSV destino
    """
    if not records:
        return
...
def request_with_retry(url: str, retries: int = MAX_RETRIES) -> Optional[requests.Response]:
    ...
    for attempt in range(retries):
        try:
            ...
            logging.warning("[HTTP %d] intento %d/%d — %s", e.response.status_code, attempt + 1, retries, url)
        except requests.exceptions.ConnectionError as e:
            logging.warning("[CONNECTION ERROR] intento %d/%d — %s", attempt + 1, retries, url)
        except requests.exceptions.Timeout:
            logging.warning("[TIMEOUT] intento %d/%d — %s", attempt + 1, retries, url)
        except Exception as e:
            logging.warning("[ERROR] intento %d/%d — %s: %s", attempt + 1, retries, type(e).__name__, e)
 
        # espera exponencial antes del siguiente intento: 2s, 4s, 8s...
        time.sleep(2 ** (attempt + 1))
 
    logging.error("[FALLIDO] Se agotaron los %d reintentos para %s", retries, url)
    return None
...
def get_league_teams(season: int,competicion_slug:str, league_code:str) -> list[dict]:
    """
    Descarga los equipos participantes en la Premier League para una temporada.

    URL ejemplo:
        https://www.transfermarkt.es/premier-league/teilnehmer/pokalwettbewerb/GB1/saison_id/{season}
    ...
    """
    url = (
        f"https://www.transfermarkt.es/{competicion_slug}"
    ...
    for row in rows:
        ...
        try:
            ...
            logging.warning("Error procesando fila de equipo: %s — href: %s", e, href)
            continue
...
def get_squad(team_slug: str, team_id: int, season: int) -> list[dict]:
    ...
    for row in rows:
        try:
            ...
            logging.warning("Error procesando jugador: %s: %s", type(e).__name__, e)
            continue
           
        except Exception as e:
            logging.error("Error inesperado en jugador: %s: %s", type(e).__name__, e)
            continue
...
def get_player_injuries(player_slug: str, player_id: str) -> list[dict]:
    ...
    for row in rows:
        try:
            ...
            logging.warning("Estructura HTML inesperada en lesiones: %s — jugador: %s", e, player_slug)
            continue
        except AttributeError as e:
            logging.warning("Elemento no encontrado en lesiones: %s — jugador: %s", e, player_slug)
            continue
        except ValueError as e:
            logging.warning("Error convirtiendo número en lesiones: %s — jugador: %s", e, player_slug)
            continue
        except Exception as e:
            logging.error("Error inesperado en lesiones: %s: %s — jugador: %s", type(e).__name__, e, player_slug)
            continue
...
def scrape_competition( league_code: str,competition_slug:str,output_dir:str, seasons:list[int]) -> None:
    """
    Orquesta la extracción completa de datos de la competicion.
    ...
    Salida ejemplo: (data/raw/transfermarkt/premier_league/):
        teams_clean.csv
        players_clean.csv
        injuries_clean.csv
    """
    ...
    teams_path    = os.path.join(output_dir, "teams_clean.csv")
    players_path  = os.path.join(output_dir, "players_clean.csv")
    injuries_path = os.path.join(output_dir, "injuries_clean.csv")
    ...
    for season in seasons:
        ...
        teams = get_league_teams(season, competition_slug, league_code)
        print(f"  {len(teams)} equipos encontrados")

        if not teams:
            logging.warning("No se obtuvieron equipos para la temporada %d", season)
            continue
...
def main():
    ...
    Lee los argumentos de la terminal:
        --competition  (obligatorio): nombre de la competición a descargar
        --seasons      (opcional):    temporadas a descargar, por defecto 2020-2024
    
    Uso:
        python -m scrapers.transfermarkt_scraper --competition "La Liga"
        python -m scrapers.transfermarkt_scraper --competition "Premier League" --seasons 2023 2024"""
    
    # Crea el objeto que va a leer los argumentos de la terminal
    parser = argparse.ArgumentParser()

    # argumento obligatorio: Nombre de la competición 
    parser.add_argument("--competition", required=True,
                        help='Ej: "La Liga", "Premier League", "Champions League"')
    
    # Argumento opcional por si solo se quiere una temporada
    parser.add_argument("--seasons", nargs="+", type=int, default=[2020,2021,2022,2023,2024])
    
    # lee los argumentos de la terminal y los guarda en args 
    # args es un objeto desde el  que acceder a los argumentos introducidos en la terminal 
    # se accede mediante notacion de punto
    args = parser.parse_args()
    
    # se llama al load_config usando el nombre de la competicion introducido en la terminal 
    try:
    ...
    tm_slug     = config["slug"]
    output_dir  = os.path.join("data", "raw", "transfermarkt", config["name"].lower().replace(" ", "_"))

    scrape_competition(league_code, tm_slug, output_dir, args.seasons)
